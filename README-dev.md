...
Расширение загружено как unpacked — Chrome не подхватывает изменения файлов автоматически.

**После правок в JS/HTML/CSS:**

1. Открыть `chrome://extensions/`
2. Найти карточку **s485s StrGenUrl**
3. Нажать кнопку обновления (иконка ↺ в левом нижнем углу карточки)
4. Закрыть и снова открыть боковую панель (она не перезагружается сама)

**Быстрый способ через клавиатуру:**

`chrome://extensions/` → нажать `R` при фокусе на карточке расширения (обновляет все unpacked расширения).

Либо через DevTools боковой панели: ПКМ → **Inspect** → в консоли `location.reload()`.

**После правок в `manifest.json` или `background.js`:**

Те же шаги — нажать ↺ на карточке. Service worker перезапустится автоматически.

**После правок в `generate-icons.js` (иконки):**

```bash
node generate-icons.js
```

Затем обновить расширение через ↺ — иконки подтянутся.
